 ...
 * A normal stderr-only diagnostic remains verbatim (apart from redaction),
 * while a mixed-channel failure identifies each retained channel.
 */
export function nativeFailureDiagnostic(
  ...
  if (channels.length === 1) {
    ...
    return failureChannelDiagnostic(channel.label, channel.value, env);
  }
  return channels
    .map(({ label, value }) => `${label}: ${failureChannelDiagnostic(label, value, env)}`)
    .join("\n");
}
...
export function redactNativeError(error: unknown, env: NodeJS.ProcessEnv = process.env): string {
  ...
  return failureChannelDiagnostic("error", message, env);
}
...
function failureChannelDiagnostic(label: string, value: string, env: NodeJS.ProcessEnv): string {
  const redacted = redactNativeDiagnostic(value, env);
  return isSafeFailureDiagnostic(label, value, redacted, env)
    ? redacted
    : contentSummary(`native-${label}`, redacted);
}

function isSafeFailureDiagnostic(
  label: string,
  value: string,
  redacted: string,
  env: NodeJS.ProcessEnv,
  allowKaifuuExtractWrapper = true,
): boolean {
  if (label === "spawn") return true;
  if (/[\u0000-\u0008\u000B\u000C\u000E-\u001A\u001C-\u001F]/u.test(value)) return false;
  if (/^[A-Za-z0-9+/_=-]{128,}$/u.test(value.trim())) return false;
  // A child-process error is generated by Node rather than by a decoder, so
  // it cannot carry source material. Every stdout/stderr channel, however,
  // is content-bearing until it identifies itself as an actual diagnostic.
  return (
    looksLikeTrustedBuildDiagnostic(value) ||
    looksLikeMissingFlagDiagnostic(value) ||
    looksLikeNativeErrorCode(value) ||
    (looksLikeStructuredNativeDiagnostic(value) && hasRedactedSpan(redacted)) ||
    looksLikeSafeNativeMetadata(value) ||
    (allowKaifuuExtractWrapper && looksLikeSafeNativeToolFailure(value, env))
  );
}

function looksLikeTrustedBuildDiagnostic(value: string): boolean {
  return /(?:^(?:cargo|rustc)\s*:|(?:^|\n)error(?:\[[A-Z]\d+\])?\s*:)/imu.test(value.trim());
}

function looksLikeMissingFlagDiagnostic(value: string): boolean {
  const normalized = value.trim().replaceAll(/\s+/gu, " ");
  const match =
    /^(?:missing|required|unknown|unrecognized|invalid)\s+(?:[A-Za-z0-9_-]+\s+){0,6}(?:flag|option|argument)\s+--[A-Za-z][A-Za-z0-9-]*(.*)$/iu.exec(
      normalized,
    );
  if (match === null) return false;
  const tail = match[1] ?? "";
  if (tail.length === 0) return true;
  return /^(?:[;:,.]?\s*(?:pass|use|provide|supply|set|include)\s+--[A-Za-z][A-Za-z0-9-]*(?:,\s*--[A-Za-z][A-Za-z0-9-]*)*(?:,\s*(?:and|or)\s+--[A-Za-z][A-Za-z0-9-]*)?[.!?]?)$/iu.test(
    tail,
  );
}

function looksLikeStructuredNativeDiagnostic(value: string): boolean {
  return /^(?:kaifuu|utsushi)(?:[._-][A-Za-z0-9_-]+)+\s*:/iu.test(value.trim());
}

function looksLikeNativeErrorCode(value: string): boolean {
  return /^(?:kaifuu|utsushi)(?:[._-][A-Za-z0-9_-]+)+$/iu.test(value.trim());
}

function looksLikeSafeNativeMetadata(value: string): boolean {
  const details = value.trim().replace(/^(?:kaifuu|utsushi)(?:[._-][A-Za-z0-9_-]+)+\s*:\s*/iu, "");
  const metadata =
    "(?:byte(?:s|_len|len)?|code|column|expected|flag|index|kind|len(?:gth)?|line|offset|path|scene|source[_-]?unit[_-]?key|start|status|unit)";
  return new RegExp(
    `^(?:${metadata}\\s*[=:]\\s*[^\\s,;]+)(?:[\\s,;]+${metadata}\\s*[=:]\\s*[^\\s,;]+)*[.?!]*$`,
    "iu",
  ).test(details);
}

function looksLikeSafeNativeToolFailure(value: string, env: NodeJS.ProcessEnv): boolean {
  const status =
    /^(?:kaifuu extract \([A-Za-z0-9_-]+\)|utsushi structure) failed with status (?:-?\d+|null):\s*([\s\S]+)$/iu.exec(
      value.trim(),
    );
  if (status !== null) {
    const inner = status[1]!;
    return isSafeFailureDiagnostic("stderr", inner, redactNativeDiagnostic(inner, env), env, false);
  }
  const spawn =
    /^(?:kaifuu extract \([A-Za-z0-9_-]+\)|utsushi structure) could not be spawned \([^\r\n]*\):\s*([\s\S]+)$/iu.exec(
      value.trim(),
    );
  return spawn !== null && looksLikeNodeSpawnDiagnostic(spawn[1]!);
}

function looksLikeNodeSpawnDiagnostic(value: string): boolean {
  return /^(?:spawn|spawnSync)\s+.+\s+(?:E[A-Z0-9_]+|ERR_[A-Z0-9_]+)$/u.test(value.trim());
}

function hasRedactedSpan(value: string): boolean {
  return value.includes(NATIVE_CONTENT_REDACTED) || value.includes(NATIVE_SECRET_REDACTED);
}
