...
// written outcome + the real billed provider-run cost. So a correction /
...
// terminal `RepairJobOutcome` from written outcomes plus typed operational
// failures — so a replay produces byte-equal results.
...
export type RepairJobExecutorDeps = {
  ...
  /**
   * Persistence sinks — the SAME `DrivenWrittenOutcomeSink` /
   ...
   * persists a written outcome the same way a first-pass driven run does.
   */
  sinks: {
...
export type RepairJobExecutionResult = {
  ...
  /** Units whose loop actually ran + persisted a written outcome. */
  unitsRun: number;
  writtenOutcomeCount: number;
  /** Units whose loop threw (isolated); no target text is fabricated. */
  failureCount: number;
  /**
 ...
 * through the real draft + QA agentic loop, persist the written outcome + the
...
export async function executeRepairJob(
  ...
  for (const resolvedInput of unitInputs) {
    ...
    if (result.status === "failed") {
      // Per-unit isolation: the loop threw; no target text is fabricated.
      failureCount += 1;
  ...
  // — recorded as `partial_failure` with failed-unit details. A successfully
  // returned loop bundle always persists a written body, regardless of QA flags
  // or repair budget annotations.
  const outcome: RepairJobOutcome =
    failureCount > 0 ? "partial_failure" : writtenOutcomeCount > 0 ? "succeeded" : "no_change";

  log(
    `repair-job-executor: job ${job.jobId} (${job.affectedScope}) ran ${unitsRun} unit(s); ` +
      `${writtenOutcomeCount} written, ${failureCount} failed -> ${outcome}`,
  );
 ...
 * failing unit has no fabricated target either way; the other units still run.
 */
 ...
 * and persist its written outcome + real billed provider-run cost. PER-UNIT
 ...
 * message) so the job's other units still run — the failing unit has no
 * fabricated target — AND the caller can record a `partial_failure` outcome
 * with the failed-unit details rather than swallowing the error after counting it.
 */
async function runRepairUnit(
  ...
  try {
    ...
    // PERSIST the written outcome FIRST. The provider-run ledger's FK
    // references the attempt the sink creates, so the outcome must land before
    // the provider-run row (mirrors the project-driven executor's ordering).
    await deps.sinks.writtenOutcome.persistWrittenOutcome(writtenOutcomeRecord);
...
export async function runRepairQueue(
  ...
  return { jobsRun, succeeded, partialFailure, noChange, totalCostUsd };
}
