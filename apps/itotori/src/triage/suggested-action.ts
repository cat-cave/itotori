  ...
  switch (rootCauseClass) {
    ...
    case "stale_context": {
      return [
        ...
        "current source revision before starting the next iteration.",
      ].join(" ");
    }
    case "source_annotation_issue": {
      return [
        "Record a canonical context correction against the upstream catalog;",
    ...
    case "glossary_conflict": {
      ...
      return [
        ...
        "record the correction for the next patch iteration.",
      ].join(" ");
    }
    case "style_guide_issue": {
      return [
        "Record the missing rule in the branch-policy wiki;",
        "add it to the branch policy and re-emit the rule pack",
    ...
    case "runtime_evidence": {
      return [
        "Attach the runtime evidence (save state, screenshot, log) to a canonical",
        "context correction or result revision; if reproducible, file a runtime-bridge",
        "bug and start the next patch iteration against the engine's latest snapshot.",
      ].join(" ");
    }
    case "unknown": {
      return [
        "Record a context correction for a human triager: no routing rule matched.",
