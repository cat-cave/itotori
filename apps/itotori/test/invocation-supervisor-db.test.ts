...
describe.skipIf(!process.env.DATABASE_URL)("InvocationSupervisor durable pause/resume", () => {
  ...
  it("releases interrupted reservations across resume cycles without blocking fresh capacity", async () => {
    const context = await isolatedMigratedContext();
    try {
      await seedScope(context.pool);
      const repository = new ItotoriLocalizationJournalRepository(context.db);
      const runId = "journal-run-resume-releases-cost-reservations";
      const firstUnitId = "raw-bridge-unit-resume-release-first";
      const secondUnitId = "raw-bridge-unit-resume-release-second";
      const thirdUnitId = "raw-bridge-unit-resume-release-third";
      const survivorUnitId = "raw-bridge-unit-resume-release-survivor";
      const replacementUnitId = "raw-bridge-unit-resume-release-replacement";
      const capUsd = "0.8"; // itotori-225-audit-allow: synthetic live-DB cap admits a replacement only after interrupted reservations release.
      const initialReservedUsd = "0.7"; // itotori-225-audit-allow: exact sum of interrupted reservations plus the unrelated unresolved survivor.
      const firstReservationUsd = "0.1"; // itotori-225-audit-allow: synthetic exact-decimal interrupted reservation fixture.
      const secondReservationUsd = "0.2"; // itotori-225-audit-allow: synthetic exact-decimal interrupted reservation fixture.
      const thirdReservationUsd = "0.3"; // itotori-225-audit-allow: synthetic exact-decimal interrupted reservation fixture.
      const survivorReservationUsd = "0.1"; // itotori-225-audit-allow: completed unknown-billing reservation must survive the resume release unchanged.
      const settledInterruptedUsd = "0.01"; // itotori-225-audit-allow: synthetic late settlement proves a released reservation never subtracts capacity twice.
      const replacementReservationUsd = "0.69"; // itotori-225-audit-allow: synthetic replacement reservation proves released capacity is admitted exactly below the cap.
      const survivorAndReplacementUsd = "0.79"; // itotori-225-audit-allow: exact active reservations after the replacement dispatch.
      const initialLease = ATOMIC_COST_LEASE;
      const initialReservations = [
        { bridgeUnitId: firstUnitId, worstCaseCostUsd: firstReservationUsd },
        { bridgeUnitId: secondUnitId, worstCaseCostUsd: secondReservationUsd },
        { bridgeUnitId: thirdUnitId, worstCaseCostUsd: thirdReservationUsd },
      ] as const;
      await repository.seedRun(ACTOR, {
        ...atomicCostSeed(runId, [
          firstUnitId,
          secondUnitId,
          thirdUnitId,
          survivorUnitId,
        ...
        costPolicy: { reservation: "resume-release-test", budgetCapUsd: capUsd },
      });

      for (const reservation of initialReservations) {
        const reserved = await repository.reserveAttemptCost(ACTOR, {
          ...atomicCostAttempt(runId, reservation.bridgeUnitId),
          worstCaseCostUsd: reservation.worstCaseCostUsd,
        });
        if (!reserved.admitted) {
          throw new Error(
            `initial reservation unexpectedly denied for ${reservation.bridgeUnitId}`,
          );
        }
      }
      const survivor = await repository.reserveAttemptCost(ACTOR, {
        ...atomicCostAttempt(runId, survivorUnitId),
        worstCaseCostUsd: survivorReservationUsd,
      });
      if (!survivor.admitted) {
        throw new Error("unrelated unresolved reservation unexpectedly denied");
      }
      const unknownSurvivorCompletion = atomicCostCompletion(survivor.attempt, {
        failureClass: "provider_network_error",
        retryDecision: "retry",
        validationResult: "semantic_invalid",
      });
      await repository.completeAttempt(ACTOR, {
        ...unknownSurvivorCompletion,
        modelId: null,
        providerId: null,
        costUsd: null,
        costKind: undefined,
      ...
      expect(await repository.loadRunCostAccount(ACTOR, runId)).toMatchObject({
        capUsd,
        spentCostUsd: "0",
        reservedCostUsd: initialReservedUsd,
      });

      const pauseBlocker = {
        kind: "provider_outage" as const,
        detail: "Synthetic lease-expiry boundary for reservation release.",
        evidence: "test:resume-release-cost-reservations",
        raisedAt: "2026-07-13T00:00:00.000Z",
        operatorAction: "Resume after the old owner has expired.",
      };
      await repository.pauseRun(ACTOR, runId, pauseBlocker, initialLease);
      await expireRunLease(context.pool, runId);

      const firstResume = await repository.resumeRun(ACTOR, runId, {
        ownerId: "resume-release-driver-b",
      });
      const firstResumeLease = {
        ownerId: "resume-release-driver-b",
        fenceToken: firstResume.fenceToken,
      } as const;
      const initialAttemptIds = initialReservations.map(
        ({ bridgeUnitId }) => atomicCostAttempt(runId, bridgeUnitId).attemptId,
      );
      expect(await repository.loadAttemptsForRun(ACTOR, runId)).toEqual(
        expect.arrayContaining(
          initialAttemptIds.map((attemptId) =>
            expect.objectContaining({
              attemptId,
              lifecycleState: "completed",
      ...
      // Interrupted is not spent: the account returns to its pre-reservation
      // balance before any later provider settlement is known.
      expect(await repository.loadRunCostAccount(ACTOR, runId)).toMatchObject({
        capUsd,
        spentCostUsd: "0",
        reservedCostUsd: survivorReservationUsd,
      });
      expect(await repository.loadCostReservations(ACTOR, runId)).toEqual(
        expect.arrayContaining([
          ...initialAttemptIds.map((attemptId) =>
          ...
          expect.objectContaining({
            attemptId: survivor.attempt.attemptId,
            reservedUsd: survivorReservationUsd,
            state: "reserved",
            reconciledUsd: null,
          }),
        ]),
      ...
      const replacement = await repository.reserveAttemptCost(ACTOR, {
        ...atomicCostAttempt(runId, replacementUnitId),
        lease: firstResumeLease,
        worstCaseCostUsd: replacementReservationUsd,
      });
      expect(replacement).toMatchObject({ admitted: true });
      if (!replacement.admitted) {
        throw new Error("replacement reservation unexpectedly denied after resume release");
      }
      expect(await repository.loadRunCostAccount(ACTOR, runId)).toMatchObject({
        capUsd,
        spentCostUsd: "0",
        reservedCostUsd: survivorAndReplacementUsd,
      });

      // A delayed exact bill for a released attempt adds only spend. It must
      // not consume the replacement reservation that is genuinely in flight.
      await repository.reconcileAttemptBilling(ACTOR, {
        runId,
        attemptId: initialAttemptIds[0]!,
        costUsd: settledInterruptedUsd,
        modelId: "atomic-cost-model",
        providerId: "atomic-cost-provider",
        usageResponseJson: { cost: settledInterruptedUsd },
      });
      await repository.reconcileAttemptBilling(ACTOR, {
        runId,
        attemptId: initialAttemptIds[0]!,
      ...
      expect(await repository.loadRunCostAccount(ACTOR, runId)).toMatchObject({
        capUsd,
        spentCostUsd: settledInterruptedUsd,
        reservedCostUsd: survivorAndReplacementUsd,
      });

      await repository.pauseRun(ACTOR, runId, pauseBlocker, firstResumeLease);
      await expireRunLease(context.pool, runId);
      const secondResume = await repository.resumeRun(ACTOR, runId, {
        ownerId: "resume-release-driver-c",
      });
      expect(secondResume).toMatchObject({ status: "running", fenceToken: 3 });
      expect(await repository.loadAttemptsForRun(ACTOR, runId)).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
