// Named real-byte oracle for the deterministic fact-snapshot pre-pass.
//
...
// the dedicated evidence lane fails loudly.
...
describe("buildFactSnapshot real primary_corpus byte oracle", () => {
  it("drives the real extract + structure seams and proves the pre-pass guarantees", () => {
    const corpus = realCorpus();
    if (!corpus) {
      throw new Error(
        "fact-snapshot real-byte proof requires the selected RealLive corpus in the private inventory",
      );
    }
    ...
    expect(Buffer.compare(readFileSync(firstSnapshotPath), readFileSync(secondSnapshotPath))).toBe(
      0,
    );
