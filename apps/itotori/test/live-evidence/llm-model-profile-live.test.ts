...
// This expensive, billable probe is intentionally registered only by the
// named `just test model-profile` evidence runner. That runner validates its
// existing credential and database inputs before this body executes.
it("certifies the live DeepSeek Flash profile through the real dispatcher", async () => {
