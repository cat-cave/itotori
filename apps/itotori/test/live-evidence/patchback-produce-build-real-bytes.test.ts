...
//
// This named live-evidence suite requires the private inventory to point at real
// RealLive installs (never committed). Missing inputs are hard failures so a CI
// lane cannot silently claim this proof ran.
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        /* Ignore an unreadable child while searching the mounted corpus. */
      }
    }
  }
  return undefined;
...
describe("produce playable build real primary_corpus byte oracle", () => {
  it("drives the real native apply and archives a real, non-empty patched build", async () => {
    if (!corpus) {
      throw new Error(
        "patchback produce real-byte proof requires the selected RealLive corpus in the private inventory",
      );
    }
    const primaryCorpus = corpus;
    ...
    const targetTextFor = (bridgeUnitId: string, index: number): string => {
      ...
      const inBodySpans = (unit?.spans ?? []).filter((s) => s.outOfBand !== true).map((s) => s.raw);
    ...
    const produced = produceNativePatchbackBuild(input, {
      sourceRoot: primaryCorpus.gameRoot,
      buildRoot,
    ...
    expect(readFileSync(patchedSeen).equals(readFileSync(primaryCorpus.seen))).toBe(false);
  ...
  it("drives itotori patch produce through the native producer for two distinct RealLive games", async () => {
    if (!corpus || !secondCorpus) {
      throw new Error(
        "multi-corpus patchback produce proof requires both selected RealLive corpora in the private inventory",
      );
    }
    const corpora = [
