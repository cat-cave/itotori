// Named real-primary_corpus oracle for the native patchback + translated-byte replay.
//
...
// When the corpus is not staged the dedicated evidence lane fails loudly.
...
describe("native patchback + replay real primary_corpus byte oracle", () => {
  it("applies accepted targets byte-exactly and Utsushi observes the target from the patched bytes", () => {
    if (!corpus) {
      throw new Error(
        "patchback real-byte proof requires the selected RealLive corpus in the private inventory",
      );
    }
    const c = corpus;
    ...
    const g00Dir = join(c.gameRoot, "REALLIVEDATA", "g00");
    ...
    const sourceSeenPath = join(c.gameRoot, "REALLIVEDATA", "Seen.txt");
