...
describe("fe-openapi-parity-all-routes: per-route teeth (all routes, request + response)", () => {
  ...
  it("covers a body for every one of the 47 routes (no route left un-teethed)", () => {
    // 47 routes: each has a response body; the mutation + reviewer/workspace
    ...
    expect(ITOTORI_API_ROUTE_IDS.length).toBe(47);
