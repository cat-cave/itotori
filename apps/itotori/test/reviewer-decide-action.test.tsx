...
//     seam `reviewer.itemAction` — approve)
...
//   2. a non-`canDecide` actor sees the decide control disabled + explained;
//   3. a failure from the API surfaces an in-strip visible error instead
//      of a silent panel.
//
...
function appliedSingleActionResult(input: {
  ...
  actorUserId: string;
  nextState: typeof reviewerQueueItemStateValues.accepted;
...
describe("ReviewerDetailScreen — decide action (canDecide)", () => {
  ...
  it("renders the Approve button for a canDecide reviewer", async () => {
    handleDetail();
    ...
    expect(
      within(strip).queryByRole("button", { name: "Queue correction" }),
  ...
  it("a non-canDecide user sees decide buttons DISABLED + EXPLAINED", async () => {
    ...
    // kind-specific Approve which also renders in the action strip.
