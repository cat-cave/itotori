// A5 dispatch boundary — the model-calling role routes deepseek-v4-flash through
// the SOLE ZDR dispatch boundary, proven offline on a recorded/memo path.
//
// No network and no Postgres: an in-memory memo store plus a recorded SSE response
// stand in for the provider. The proofs show the A5 call spec is the certified A5
// route (no provider named, ZDR policy, wiki-object terminal, ZERO tools), that the
// public dispatch entry asserts that certified route in EVERY mode — test-dev
// included — and rejects a forged spec BEFORE the transport, that a recorded voice-
// profile draft returns through dispatch(), and that the production caller maps it.
...
/** A genuine recorded voice-profile object the "provider" returns — a valid
 * profile with a base register, one route-scoped counterpart rule, and one route-
 * scoped arc-register shift, all decode-grounded. */
function recordedVoice(
  model: ReturnType<typeof buildClaimFixture>["model"],
  request: A5VoiceRequest,
): WikiObject {
  const window = request.occurrenceUnitIds;
  return assembleVoiceProfile(model, CONTEXT, request.evidence, request.counterpartIds, {
...
describe("A5 dispatches through the sole ZDR boundary", () => {
  it("PROOF: the A5 call spec is the certified, no-provider-named, wiki-object route with ZERO tools", () => {
    const { model, request } = voiceRequest();
    const { spec } = buildA5CallSpec(model, CONTEXT, request);
    expect(spec.roleId).toBe("A5");
  ...
  it("PROOF: assertCertifiedRoute passes the real spec and REJECTS a forged one in EVERY mode (test-dev)", () => {
    ...
    expect(() => assertCertifiedRoute(noZdr)).toThrow(A5RoleError);
  ...
  it("PROOF: dispatchA5 asserts the certified route BEFORE the transport (forged spec, zero fetches)", async () => {
    const { model, request } = voiceRequest();
    ...
    await expect(dispatchA5(wrongModel, prompts, configured)).rejects.toThrow(
      /route-not-certified/u,
    );
    expect(fetches).toBe(0);
  });

  it("PROOF: a recorded voice-profile draft returns through dispatch()", async () => {
    const { model, request } = voiceRequest();
    const { spec, prompts } = buildA5CallSpec(model, CONTEXT, request);
    let fetches = 0;
    ...
    const result = await dispatchA5(spec, prompts, configured);
    expect(result.status).toBe("success");
    expect(result.status === "success" ? result.value.kind : null).toBe("voice-profile");
  ...
  it("PROOF: the production caller maps the dispatched draft", async () => {
    const { model, request } = voiceRequest();
    const configured = runtime([structuredProviderResponse(recordedVoice(model, request))]);
    const caller = dispatchingA5Caller(model, CONTEXT, configured);
    const draft = await caller(request);
    expect(draft.base.register).toBe("だ・である（素）");
    expect(draft.counterparts[0]!.counterpartId).toBe("nam-22");
    expect(draft.counterparts[0]!.evidenceId).toBe(request.occurrenceUnitIds[0]);
