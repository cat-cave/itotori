...
const getDashboardStatus = vi.fn(async () => ({
  projectId: "project-1",
...
vi.mock("@itotori/db", () => ({
  createDatabaseContext: vi.fn(() => ({ db: {}, close })),
  ItotoriProjectRepository: class {
...
async function requestJson(path: string): Promise<unknown> {
  const server = createItotoriServer({ webRoot: new URL("file:///tmp/itotori-empty-web/") });
