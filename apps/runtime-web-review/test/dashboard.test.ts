...
describe("Utsushi runtime dashboard", () => {
  ...
  it("distinguishes content-backed hashes from repository-generated placeholder hashes", async () => {
    // UTSUSHI-136 — a placeholder hash is structurally a valid managed link,
    // but it is NOT authentic content evidence. The dashboard must render the
    // link AND badge it as a generated placeholder so proof manifests do not
    // overstate artifact evidence strength. Content-backed hashes render as a
    // plain link with a content provenance badge.
    currentFixture = runtimeFixture("repository-fallback-hash");
    const root = document.createElement("div");
    document.body.append(root);

    await renderRuntimeDashboard(root, "http://itotori.test/api/runtime/v0.2/status");

    // Both artifacts still render as managed artifact links. The trace event
    // row also links to the trace artifact, so the page carries 3 anchors.
    const links = [...root.querySelectorAll<HTMLAnchorElement>('a[href^="/artifact-store/"]')];
    expect(links).toHaveLength(3);

    // The trace artifact is content-backed; the screenshot is a fallback.
    const provenances = [...root.querySelectorAll<HTMLElement>("[data-provenance]")];
    expect(provenances.map((el) => el.dataset.provenance)).toEqual(
      expect.arrayContaining(["content", "repository_fallback"]),
    );
    expect(provenances.some((el) => el.dataset.provenance === "content")).toBe(true);
    expect(provenances.some((el) => el.dataset.provenance === "repository_fallback")).toBe(true);

    // The fallback artifact must carry the inline placeholder badge alongside
    // its link so it cannot be mistaken for content proof.
    expect(root.textContent).toContain("generated placeholder hash");
    expect(root.textContent).toContain("sha256:repository-fallback-screen");
  });

  it("renders content-backed hashes without a placeholder badge", async () => {
    // UTSUSHI-136 — content-backed hashes must NOT carry the placeholder
    // badge; they are authentic adapter evidence and render as plain links.
    currentFixture = runtimeFixture("passed-e2-capture");
    const root = document.createElement("div");
    document.body.append(root);
    ...
    expect(root.textContent).not.toContain("generated placeholder hash");
    const provenances = [...root.querySelectorAll<HTMLElement>("[data-provenance]")];
    expect(provenances.length).toBeGreaterThan(0);
    expect(provenances.every((el) => el.dataset.provenance === "content")).toBe(true);
  });
...
function runtimeFixture(
  ...
  if (state === "repository-fallback-hash") {
    // UTSUSHI-136 — the screenshot artifact's hash is a deterministic
    // placeholder generated by the repository from managed-artifact metadata,
    // not authentic adapter content evidence. The dashboard must surface it
    // as a generated placeholder hash rather than content proof.
    return {
      ...base,
      artifacts: base.artifacts.map((artifact) =>
        artifact.artifactId === screenshotArtifactId
