...
describe("Utsushi runtime dashboard", () => {
  ...
  it("distinguishes content-backed hashes from repository-generated placeholder hashes", async () => {
    // A placeholder hash is structurally a valid managed link, but it is NOT
    // authentic content evidence. The dashboard must render the link AND
    // badge it as a generated placeholder so proof manifests do not overstate
    // artifact evidence strength. Content-backed hashes render as a
  ...
  it("renders content-backed hashes without a placeholder badge", async () => {
    // Content-backed hashes must NOT carry the placeholder badge; they are
    // authentic adapter evidence and render as plain links.
    currentFixture = runtimeFixture("passed-e2-capture");
    const root = document.createElement("div");
    document.body.append(root);
