...
fn run_with_args_and_registry(
    ...
    match args.first().map(String::as_str) {
        ...
        _ => {
            return Err(
                "usage: kaifuu <detect|extract|asset-inventory|patch|diff|apply|verify|golden|offset-map|helper|helper-result|key-helper|helper-registry|key|siglus|rpgmaker|rpg-maker|xp3|wolf|profile|readiness|capabilities|binary-patch-smoke|compat-evidence|asset-ocr|vault> ..."
                    .into(),
            );
        }
    }
