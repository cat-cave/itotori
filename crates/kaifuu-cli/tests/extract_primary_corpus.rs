...
//! `kaifuu-cli extract --engine reallive --scope unit-set --unit-ids 2011 --bundle-output PATH`.
//! Env-gated on `private inventory row`. Runs the kaifuu-cli
//! binary against the selected private extracted root, asserts the
...
fn cli_extract_engine_reallive_dialogue_scene_writes_schema_valid_v02_bundle() {
    ...
    let bundle_out = tmp_dir.path().join("primary-corpus-scene-2011.bridge.json");
    ...
    cmd.arg("extract")
        ...
        .arg("kaifuu-reallive-primary-corpus")
        .arg("--source-locale")
        .arg("ja-JP");
...
/// `extract --scope all` over the entire selected source archive — every
/// populated scene decodes (decode-100), producing ONE multi-scene v0.2 BRIDGE.
/// `kaifuu extract --scope all` produces the BRIDGE only — NOT the replay
...
/// (deps flow utsushi → kaifuu). The native structure proof lives on the
...
fn cli_extract_whole_seen_primary_corpus_writes_multi_scene_bridge() {
    ...
    let tmp_dir = tempfile::tempdir().expect("tmp dir");
    let bundle_out = tmp_dir.path().join("primary-corpus-whole.bridge.json");
    let report_out = tmp_dir
        .path()
        .join("primary-corpus-whole.decompile-report.json");
    ...
    assert!(
        output.status.success(),
        "kaifuu-cli --scope all exited non-zero: status={:?}\nstdout={}\nstderr={}",
        output.status,
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr),
    );
