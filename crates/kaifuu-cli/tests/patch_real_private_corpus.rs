...
//! binary against the selected private extracted root, asserts:
//! - The command exits 0.
...
#[path = "patch_real_private_corpus/archive.rs"]
mod archive;
...
use kaifuu_reallive::{
    ...
    encode_shift_jis_slot, parse_real_bytecode,
};

use archive::{decompress_scene, recover_archive_xor2_cipher, scene_blob_bytes};
...
fn cli_patch_engine_reallive_writes_patched_seen_txt_under_writable_target() {
    ...
    let xor2_cipher = recover_archive_xor2_cipher(&source_seen_bytes, &src_index)
        .expect("pristine source must yield a validated xor_2 cipher");
