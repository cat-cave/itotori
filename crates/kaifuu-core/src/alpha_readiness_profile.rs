...
use crate::{
    AdapterCapabilityMatrix, CapabilityLevel, CapabilityLevelStatus, KaifuuResult,
    LayeredAccessHelperStatus, LayeredAccessKeyMaterialStatus, OperationStatus, PackedEngineFamily,
    PartialDiagnosticSeverity, PatchBackTransform, SecretRef, redact_for_log_or_report,
    stable_json,
};
...
pub use report::{
    AlphaReadinessValidationReport, validate_alpha_readiness_dir, validate_alpha_readiness_profiles,
};
...
impl AlphaReadinessProfile {
    fn synthetic_adapter_id(family: PackedEngineFamily) -> String {
...
pub fn alpha_readiness_seed_bgi() -> AlphaReadinessProfile {
    AlphaReadinessProfile {
        ...
        source_node_id: ALPHA_READINESS_SOURCE_NODE_ID.to_string(),
        prerequisite_proof: "KAIFUU-041.bgi-detection".to_string(),
        engine_family: PackedEngineFamily::Bgi,
        capabilities: AdapterCapabilityMatrix::identify_only(
            AlphaReadinessProfile::synthetic_adapter_id(PackedEngineFamily::Bgi),
            "detector/profile evidence only (KAIFUU-041); no archive parser or patch support",
        ),
        helper_key: AlphaHelperKeyStatus::none_required(),
        patch_back: PatchBackTransform::Unsupported,
        provenance: AlphaReadinessProvenance::public_synthetic(),
