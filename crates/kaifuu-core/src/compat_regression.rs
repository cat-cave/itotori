...
use crate::compat_profile::{
    ClaimedSupportEntryReport, CompatDiagnostic, validate_claimed_support_tuple,
};
...
// The runner — the per-tuple + aggregate driver lives in its own child module
// so the catalogue / baseline / finding / result-artifact types above stay
// co-located while the re-run machinery reads as one cohesive band.
mod runner;

pub use runner::run_claimed_profile_regression;
