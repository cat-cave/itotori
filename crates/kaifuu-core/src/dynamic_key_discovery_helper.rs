//! KAIFUU-065 — Dynamic-key-discovery REMOTE HELPER boundary (reference tier).
//!
...
//!   scanned memory. Every artifact deep-scans (KAIFUU-036/094 pattern) for raw
...
/// The stable, non-secret platform id surfaced in the KAIFUU-085 helper result
...
/// Semantic code: a nested helper result failed KAIFUU-085 schema validation.
...
impl DynamicKeyDiscoveryRequest {
    ...
    /// material or a local path (deep-scan, KAIFUU-036/094 pattern), and the
...
impl DynamicKeyDiscoveryResponse {
    ...
    ///    nothing;
    /// 3. the nested helper result must pass KAIFUU-085 schema validation.
    pub fn validate(&self) -> DynamicKeyDiscoveryValidation {
        let mut failures = Vec::new();
...
impl DynamicKeyDiscoveryRefusal {
    ...
    /// and its nested helper result must pass KAIFUU-085 validation.
    pub fn validate(&self) -> DynamicKeyDiscoveryValidation {
        let mut failures = Vec::new();
...
///   [`SEMANTIC_DYNAMIC_KEY_HELPER_DISABLED`] diagnostic and a KAIFUU-085 helper
...
/// Validates a nested `helperResult` object (if present) against the KAIFUU-085
...
mod tests {
    ...
    fn live_opt_in_enables_and_resolves_ref_plus_proof() {
        ...
        // The nested helper result is a valid KAIFUU-085 remote-helper success.
        assert_eq!(
