//! Synthetic Siglus known-key parser-boundary smoke.
//! Validates key-ref plumbing and parser diagnostics for fixture inputs; does
//! not claim production Siglus extraction, decryption, patch-back, or runtime
//! compatibility.
...
use crate::{
    FIXTURE_HELPER_ALLOWLIST_REF_ID, FIXTURE_HELPER_REGISTRY_ID, HelperCapability,
    HelperRedactionStatus, HelperRegistryInvocationRequest, KaifuuResult, OperationStatus,
    ProofHash, SEMANTIC_HELPER_REQUIRED, SEMANTIC_KEY_IMPORT_WRONG_ENGINE_PROFILE,
    SEMANTIC_KEY_VALIDATION_FAILED, SEMANTIC_MISSING_KEY_MATERIAL,
    SEMANTIC_SIGLUS_UNSUPPORTED_OPCODE, SIGLUS_PARSER_BOUNDARY_SCHEMA_VERSION, SecretRef,
    fixture_helper_registry, redact_for_log_or_report, sha256_file_ref, stable_json,
};
    ...
    if request.variant == SiglusParserBoundarySmokeVariant::HelperRequired {
        return Ok(siglus_parser_boundary_report(
            ...
            vec![SiglusParserBoundaryDiagnostic {
                ...
                message: "parser-boundary smoke requires a key-ref helper request"
                    .to_string(),
