...
///
/// This is the **single source of truth** for the mask across the crate:
/// `compressor.rs` (encode side), the `scene_1_dispatch_real_bytes`
/// integration test, and the `probe_scene_1_encryption` example all
/// reference this one const so a transcription error cannot diverge
/// encode from decode.
pub const AVG32_XOR_MASK: [u8; 256] = [
