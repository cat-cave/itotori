use super::super::{GameexeKeyFamily, GameexeKeyTreatment, UnknownReason};

pub(super) fn classify_tail(key: &str, value: &str) -> (GameexeKeyFamily, GameexeKeyTreatment) {
    let bare = &key[1..];
    ...
    if let Some(rest) = bare.strip_prefix("HINT.") {
        let (kind, rest_after) = super::split_first_dot(rest);
        return (
    ...
    if bare.starts_with("G00") {
        ...
        let treatment = if super::is_numeric_config_value(value) {
            GameexeKeyTreatment::Config
        } else {
            GameexeKeyTreatment::AssetReference
        };
...
fn parse_koeonoff(rest: &str) -> GameexeKeyFamily {
    ...
    let (index, after_index) = super::split_first_dot(rest);
