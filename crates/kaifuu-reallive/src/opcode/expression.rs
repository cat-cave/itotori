...
/// remain.
pub(super) fn read_i32_le(bytes: &[u8], pos: usize) -> Result<i32, RealLiveParseError> {
    if pos + 4 > bytes.len() {
...
pub(super) fn is_expr_token_lead(byte: u8) -> bool {
    matches!(
        byte,
...
pub(super) fn is_nonstring_data_lead(byte: u8) -> bool {
...
/// data item — across the observed and second validated archives that item is always
...
pub(super) fn parse_special_param(
    bytes: &[u8],
    pos: usize,
    depth: usize,
) -> Result<(Expr, usize), RealLiveParseError> {
...
pub(super) fn parse_expression_at_depth(
    bytes: &[u8],
    pos: usize,
    depth: usize,
) -> Result<(Expr, usize), RealLiveParseError> {
...
pub(super) fn ensure_expression_depth(
    bytes: &[u8],
    pos: usize,
    depth: usize,
...
/// the string early.
pub(super) fn string_operand_len(bytes: &[u8], pos: usize) -> usize {
    let mut i = pos;
...
pub(super) fn is_string_operand_delimiter(byte: u8) -> bool {
    matches!(
        byte,
        opener::META_COMMA
            | opener::META_LINE
