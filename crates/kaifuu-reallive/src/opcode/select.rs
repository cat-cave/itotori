//! `module_sel` SelectElement framing + NextString option text.
//!
//! Choice-option decode (`{ … }` blocks) and the NextString-SAFE encoder
//! used by patchback. Split out of `opcode.rs` to keep that file under its
//! line cap; behaviour is unchanged.

use super::{
    COMMAND_HEADER_LEN, CommandArg, EXPR_PAREN_CLOSE, EXPR_PAREN_OPEN, RealLiveParseError, opener,
    parse_expression,
};
