//! RealLive ExpressionPiece grammar + evaluator.
//!
//! Typed [`Expr`] tree plus the recursive-descent [`parse_expression`]
//! decoder for `0x24` Expression elements and Command argument lists,
//! computing each node's exact byte span so the bytecode stream stays
//! aligned. Split out from `opcode.rs` to keep that file under its line
...
use crate::opcode::{RealLiveParseError, is_shift_jis_textout_lead, opener};
...
pub(crate) const EXPR_INT_LITERAL: u8 = 0xFF;
/// ExpressionPiece store-register reference (`0xC8`).
pub(crate) const EXPR_STORE_REGISTER: u8 = 0xC8;
/// Memory-reference index open / close brackets (`[` `]`).
pub(crate) const EXPR_INDEX_OPEN: u8 = 0x5B;
pub(crate) const EXPR_INDEX_CLOSE: u8 = 0x5D;
/// Sub-expression grouping parentheses (`(` `)`).
pub(crate) const EXPR_PAREN_OPEN: u8 = 0x28;
pub(crate) const EXPR_PAREN_CLOSE: u8 = 0x29;
...
pub(crate) const EXPR_DOLLAR: u8 = 0x24;
...
pub(crate) const EXPR_SPECIAL: u8 = 0x61;
...
pub(crate) fn is_special_param_lead(bytes: &[u8], pos: usize) -> bool {
...
pub(crate) fn parse_data(bytes: &[u8], pos: usize) -> Result<(Expr, usize), RealLiveParseError> {
    match bytes.get(pos) {
...
pub(crate) fn parse_token(bytes: &[u8], pos: usize) -> Result<(Expr, usize), RealLiveParseError> {
