...
fn empty_choice_option_keeps_later_unit_splice_aligned_end_to_end() {
    ...
    let produced =
        crate::produce_bundle(1, &[0u8; 32], &plaintext, &report, &opts).expect("bundle builds");
    ...
    let translated = TranslatedBundleV02::from_json(&translated_value).expect("translated parses");
...
fn length_changing_edit_recalculates_goto_target() {
    for target_text in ["HELLO WORLD FROM KAIFUU PATCHBACK", "A"] {
        ...
        let bundle_json = make_bundle_json(REALLIVE_SEEN_TXT_DIRECTORY_BYTE_LEN, 0, 2, target_text);
        let bundle = TranslatedBundleV02::from_json(&bundle_json).expect("bundle parses");
        let patched = apply_translated_bundle(
            &archive,
            &bundle,
        ...
        let spans = parse_real_bytecode(&new_decompressed).expect("patched bytecode re-decodes");
        ...
        for op in &spans {
            ...
            let (_o, w) =
                crate::opcode::decode_element(&new_decompressed, cursor).expect("element decodes");
            cursor += w;
        }
        assert!(
            lands_on_metaline,
