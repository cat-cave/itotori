...
//! staged corpus (`private inventory row` = Sweetie HD 1.6.x, plus
//! `private inventory row` = Kanon 1.2.6.x when staged) and enforces
...
//! `private inventory row` is set; otherwise an absent corpus is an
...
fn classifies_every_staged_gameexe_ini_to_zero_unknown() {
    ...
    // private inventory row is set). Each must reach zero-unknown.
