...
//! RealLive engine-family behaviour must validate
...
//! [`real_corpus::REAL_GAME_ROOT_2_ENV`] (`private inventory row`).
...
#[path = "multi_corpus_real_bytes/cases.rs"]
mod cases;
...
/// Known Sweetie HD (`private inventory row`, corpus-1) SEEN sha256, recorded
...
/// so `private inventory row` accidentally re-pointed at Sweetie HD is caught
...
/// needs BOTH corpora staged), this test gates on `private inventory row`
