...
pub(crate) fn drive(
    ...
    let report = json!({
        ...
        "pixelOcr": {
            // This value is generated by the render pipeline reopening the
            // persisted PUBLIC PNG and matching its pixels against a fixed
            // font alphabet. It is intentionally independent from this
            // command's replay text and --expect-text-contains assertion.
            "text": &shots.ocr.text,
            "status": shots.ocr.status.as_str(),
            "recognizedGlyphCount": shots.ocr.recognized_glyph_count,
            "unrecognizedGlyphCount": shots.ocr.unrecognized_glyph_count,
            "frameSha256": &shots.ocr.frame_sha256,
            "source": "emitted-public-png",
        },
        "expectTextContains": params.expect_text_contains,
        "pixelGate": render_validate_pixel_gate::scene_verdict(
            shots.is_incomplete(),
            shots.skipped_objects.len(),
            shots.decode_warnings.len(),
        ),
