...
pub fn require_real_bytes(test_name: &str) {
    panic!(
        "real-bytes coverage is STRICT: {REAL_GAME_ROOT_ENV} unset or no REALLIVEDATA directory \
         found; {test_name} did not exercise real bytes (re-run with \
         {REAL_GAME_ROOT_ENV}=/path/to/reallive-game-root; these suites run in the periodic \
         ground-truth oracle where the corpora are staged)."
    );
}
