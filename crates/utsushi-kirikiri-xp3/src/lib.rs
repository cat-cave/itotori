...
// `EnginePort::capture` return type. `CaptureOutcome` is currently reachable
// only at the crate root — the substrate facade does not yet re-export it — so
// the same documented reach-around the `utsushi-rpgmaker-mv-mz`,
// `utsushi-siglus`, and `utsushi-reallive` scaffolds carry is repeated here
// (renamed to make the audit site grep-pinnable). A future facade revision
// that lifts `CaptureOutcome` into `utsushi_core::substrate` drops it from all
// ports together.
