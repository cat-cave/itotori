...
/// intra-scene byte offsets, so the NarrativeStructure field emits only scene
/// ids after this bounded local walk resolves a real scene-level transfer.
        ...
        scene.insert(
            ...
            json!(
                decompressed
                    ...
                    .map(|s| dispatch_fanout_scenes(&s.bytecode))
                    .unwrap_or_default()
            ),
        );
