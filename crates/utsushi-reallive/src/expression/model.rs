...
/// Open-paren byte (`(`).
pub(super) const PAREN_OPEN: u8 = b'(';
/// Close-paren byte (`)`).
pub(super) const PAREN_CLOSE: u8 = b')';
/// Open-bracket byte (`[`).
pub(super) const BRACKET_OPEN: u8 = b'[';
/// Close-bracket byte (`]`).
pub(super) const BRACKET_CLOSE: u8 = b']';
...
pub fn parse_expression(bytes: &[u8]) -> Result<(ExprNode, usize), ExpressionParseError> {
    let parsed =
        super::parser::parse_expression_inner(bytes, /*recover_unknown_operators=*/ false)?;
...
pub fn parse_expression_with_warnings(
    ...
    super::parser::parse_expression_inner(bytes, /*recover_unknown_operators=*/ true)
}
