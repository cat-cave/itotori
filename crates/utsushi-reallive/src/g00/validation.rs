//! Strict LZSS framing/token validation and metadata probing for g00 streams.
//!
//! Allocation-free walk of the LZSS token stream
//! ([`validate_g00_lzss_content`], [`walk_lzss_counts`]) plus typed metadata
//! accessors ([`probe_g00_pattern_geometry`]). These complement the best-effort
//! [`super::decode_g00`] decoder with a strict, no-allocation framing check.

use super::{
    G00_HEADER_PREAMBLE_BYTE_LEN, G00_REGION_RECORD_BYTE_LEN, G00_TYPE0_BGR_BYTES_PER_PIXEL,
    G00_TYPE2_MAX_TRAILING_PADDING, G00Type, LzssVariant,
};
