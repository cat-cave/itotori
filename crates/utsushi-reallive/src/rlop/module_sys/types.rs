...
use super::super::super::RlopKey;

pub const SYS_MODULE_TYPE: u8 = 1;
pub(super) const LATTICE_TYPES: [u8; 3] = [0, 1, 2];
...
pub(super) const ARITH_ALIASES: &[(SysOpcode, u16)] = &[
    (SysOpcode::Rnd, 1000),
    (SysOpcode::Pcnt, 1001),
    (SysOpcode::Abs, 1002),
...
/// under.
pub(super) const RNG_STATE_PATH: &str = "port.sys_runtime.rng_state";
/// State-path leaf the manifest entry lives under so the empty-tree
/// rejection on the substrate side does not bite us.
pub(super) const MANIFEST_PATH: &str = "port.sys_runtime.manifest";
/// Stable manifest string.
pub(super) const SYS_RUNTIME_MANIFEST: &str = "utsushi-reallive-sys-runtime/0.1.0-alpha";
