...
/// An end opcode has no invented successor. This is precisely the feasibility
/// question: can SCRIPT.SRC itself get to a real message syscall?
    ...
    while let Some(state) = queue.pop_front() {
        ...
        if matches!(instruction.family, CommandFamily::TextShow { .. }) {
            return CfgResult {
                ...
                edges,
                max_return_depth,
                first_message: Some(instruction.offset),
            };
        }
        let next = state.ip + 1;
        ...
        match instruction.opcode.id() {
            ...
            _ => successors.push(ControlState {
                ip: next,
                returns: state.returns.clone(),
            }),
        }
...
fn classifies_script_only_message_feasibility_and_the_pre_halt_call_contract() {
    ...
    for (index, root) in CORPORA.iter().enumerate() {
        ...
        assert_eq!(
            cfg.first_message,
            None,
            "corpus {} has no script-only path from entry to a message call; an end opcode cannot acquire an invented successor",
            index + 1
        );
        ...
        eprintln!(
            "[corpus {}] cfg_states={} cfg_edges={} max_return_depth={} first_message={:?} static_messages={} executed_calls={} call_fields(category/function/stack/destination/return/bank_writes)={}/{}/{}/{}/{}/{} trace_branches={} branch_target_offsets={} attachment={:?} first32={:04x?} final32={:04x?} post_halt_script_reads=0",
            index + 1,
            cfg.states,
            cfg.edges,
