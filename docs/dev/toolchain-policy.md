...
| Rust runtime          | `rust-toolchain.toml`                                                                                                    | Pins an EXACT stable release (e.g. `1.97.0`), not a floating `stable`. Cargo remains the build, test, clippy, and dependency authority for Rust crates. CI's immutable `dtolnay/rust-toolchain` v1 commit's `toolchain:` input must equal this pin. |
| CI behavior           | `.github/workflows/pr-tiers.yml`, `_tier0.yml`, `_tier1.yml`, `.github/actions/setup-itotori/action.yml`, and `justfile` | CI pins toolchains via the setup composite and invokes the `just ci <lane>` delegates documented in `ci-lanes.md`.                                                                                                                                  |
...
6. Update the Rust toolchain pin: bump `rust-toolchain.toml`'s exact `channel`
   ...
   `.github/actions/setup-itotori/action.yml`. Keep the two in lockstep.
