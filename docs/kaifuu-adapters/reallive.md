...
smoke parser the historical the relevant capability/173 addenda below describe. As delivered
through the relevant capability + the relevant capability and the semantic-command-cataloguing work, the
...
record (the relevant capability detector → 173 parser smoke → 174 inventory + patch-back →
...
- Roadmap node: the relevant capability (detector); successor scopes the relevant capability (Scene/SEEN parser-boundary smoke), the relevant capability (text inventory adapter), the relevant capability (runtime port). the relevant capability establishes only the identify/inventory boundary.
- Owner: kaifuu engine-research track.
- Adapter id: `kaifuu.reallive`
- Crate or module: `kaifuu-engine-fixture` (struct `RealLiveProfileDetectorAdapter`); archive-matrix row `reallive-seen-txt` in `kaifuu-core`. A dedicated `kaifuu-reallive` crate is deferred to the relevant capability/174 once the parser/extractor lands.
- Engine family: RealLive (VisualArt's / Key — same VM lineage as AVG32 and Siglus, but distinct on-disk shape).
- Supported versions and variants: synthetic detector fixtures only at this the relevant capability detector slice (SUPERSEDED — see **Current state**: the real Sweetie HD + Kanon full archives are now positive, zero-unknown decompile evidence, not just detector fixtures). The detector accepts both a synthetic short-circuit (SEEN.TXT and Gameexe.ini matching the synthetic magic bytes) and a generic real-shape envelope (SEEN.TXT little-endian count + offset table fits inside file length AND Gameexe.ini contains at least one RealLive-specific key prefix). Real-game disambiguation is exercised in CI through synthetic positive + Siglus-cross / AVG32-cross negative fixtures; real RealLive titles (including the ALPHA-006 vertical Sweetie HD Remaster + Sweets) become positive evidence after the relevant capability ships and are exercised at ALPHA-006.
- Explicitly excluded versions and variants:
  ...
  - Encrypted SEEN.TXT or protected Gameexe variants → outside the relevant capability; future encrypted RealLive support is a separate node and requires a key-profile boundary review.
...
- Unsupported or gated boundary: Scene/SEEN bytecode decode, `.koe`/`.nwk`/`.ovk` voice extraction, `.g00` image rebuild, Gameexe.ini patch-back, RealLive VM replay. All return `kaifuu.unsupported_layered_transform` until the relevant capability / the relevant capability / the relevant capability land.
- Public fixture ids:
...
- Fixture license and attribution: synthetic, CC0-1.0. No retail bytes, no corpus-vault access in the relevant capability (vault-source adapter is read-only and is exercised at ALPHA-006, not at the detector node).
...
- Required round-trip artifacts: not applicable at the relevant capability (detector only). Round-trip is the relevant capability's responsibility.
- Byte-identical or normalized equivalence rule: deferred to the relevant capability.
- Supported encodings and newline rules: Gameexe.ini is read as bytes (up to ~64 KiB) and matched only against ASCII key prefixes (`#GAMEEXE_VERSION`, `#REGNAME`, `#G00*`, `#KOE*`, `#SEEN*`). Shift-JIS handling is a the relevant capability concern.
- Text surfaces: deferred to the relevant capability.
...
- Semantic capability errors:
  - `kaifuu.ambiguous_engine_variant` (new in the relevant capability; see `crates/kaifuu-core/src/lib/semantic_error.rs` SemanticErrorCode catalog and §9.1 of the implementation plan).
  - `kaifuu.unsupported_engine_variant` (new in the relevant capability).
...
- License review decisions:
  ...
  - Format observations against the ALPHA-006 vault title → derived from publicly observable file layout; logged as `private-local-only` aggregate evidence at the ALPHA-006 vertical, not encoded into the relevant capability code.
- Parser spike status: not applicable (no parsing in the relevant capability). Parser spike begins under the relevant capability.
...
- Key profile requirements: none for the alpha-vertical title's SEEN.TXT / Gameexe.ini path. `.ovk`/`.koe` voice obfuscation is a the relevant capability / the relevant capability concern.
- Helper requirements: **none**. Per the playbook's per-game evidence-first rule, the relevant capability ships as pure static detection. If a future claimed game proves static detection insufficient, that's a separate node — not part of the relevant capability.
...
- Known gaps and proposed P2/P3 follow-ups:
  - the relevant capability — Scene/SEEN parser-boundary smoke.
  - the relevant capability — text inventory adapter (Scene/SEEN/Gameexe text slots, protected markup, asset references, patch-back).
  - the relevant capability — native RealLive runtime port (rlvm research anchor).
...
- Initial support boundary (parser scope) **[SUPERSEDED — see Current state: the the relevant capability/211 semantic catalogue decodes both real full archives zero-unknown; the eight-opcode set below was the the relevant capability smoke slice only]**: smoke — single fixture-safe scene per archive, eight named opcodes (`TextDisplay`, `SetSpeaker`, `Choice`, `SetVar`, `Jump`, `Return`, `ClearScreen`, `Pause`), and a documented synthetic instruction shape (opener `0x23`, opcode byte, operand-count byte, then `i`/`s`/`l` operand tags). Unrecognized opener bytes and opcodes emit `kaifuu.reallive.unrecognized_instruction` warnings paired with an `Unrecognized` AST node carrying the raw opener — never silent skip.
- Unsupported or gated boundary at this slice: real-game variability beyond the synthetic fixture (per-scene headers, real-game-only operand shapes, the long tail of RealLive opcodes); Shift-JIS decode (the relevant capability codec stage); encrypted SEEN.TXT (future node); patch-back (the relevant capability); jump resolution / scene-graph linking / expression evaluation / VM execution (the relevant capability); CLI inventory subcommand (the relevant capability). All emit `kaifuu.unsupported_layered_transform` or stay outside the parser surface entirely.
- Public fixture ids: `smoke-scene-001`, `truncated-scene-001`, `unknown-opcode-001` (crate-local under `crates/kaifuu-reallive/tests/fixtures/`; **not** promoted to `fixtures/public/manifest.schema.json` per §9.2 of the plan — the relevant capability may promote them when bridge-bundle goldens land).
- Public fixture source class: synthetic.
...
- Fixture license and attribution: synthetic, CC0-1.0. No retail bytes, no corpus-vault access in the relevant capability.
- Semantic capability errors (parser-local, new in the relevant capability):
  - `kaifuu.reallive.invalid_archive_envelope` (Fatal; maps to `kaifuu.unknown_engine_variant` at the the relevant capability adapter boundary).
...
- Parser spike status: completed under the relevant capability. Spike outcome rolled directly into the smoke fixtures (no separate spike artifact).
- Local validation commands:
...
- Known gaps (P2/P3 follow-ups):
  - the relevant capability — text inventory adapter (Scene/SEEN/Gameexe text slots, Shift-JIS decode, protected markup, asset references, patch-back, `EncodedStringSlot` projection from the parser AST).
  - the relevant capability — native RealLive runtime port (opcode execution semantics, jump resolution, VM port).
  ...
  - Offset-map / logical-id layer for patch-back-stability (the byte-position-derived string-slot ids in this slice are intentionally physical-position ids; the logical layer is the relevant capability's offset-map scope).

### the relevant capability rlvm clean-room worker checklist
...
## the relevant capability text inventory adapter addendum

- Roadmap node: the relevant capability.
...
- Asset inventory surfaces: top-level files (carry forward from the relevant capability) plus per-StringSlot asset refs (`.g00`, `.koe`, `.ovk`, `.nwk` by extension) and per-Gameexe-key asset refs (documented catalogue keys only).
...
- Patch-back diagnostic codes — the canonical set is emitted by the bundle-driven driver `apply_translated_bundle` (`crates/kaifuu-reallive/src/patchback/bundle_driven.rs`), all Fatal. The older the relevant capability slot-API codes (`patchback_offset_overflow`, `patchback_unsupported_length_policy`, `patchback_unknown_slot_id`, `patchback_stale_source_hash`, `patchback_protected_span_lost`, `patchback_parser_regression`) are **RETIRED**: length-changing edits succeed through the bundle-driven path (offset table rewritten, goto pointers recalculated), so there is no `patchback_offset_overflow`.
...
- Parser spike status: completed under the relevant capability.
- Local validation commands:
  - `cargo test -p kaifuu-reallive`
  - `cargo test -p kaifuu-engine-fixture`
  - `cargo test -p kaifuu-core`
...
- Known gaps (P2/P3 follow-ups):
  - ~~Length-changing patch-back (offset-table rewrite + jump-target recalculation).~~ **DELIVERED** by the bundle-driven `apply_translated_bundle` driver — see the the relevant capability length-changing addendum below. The `reallive-patchback-length-changing` node proved it on Sweetie HD (longer + shorter bodies, offset table rewritten, all 91 goto pointers of scene 8509 recalculated to boundary-landing offsets, zero-unknown re-decompile). The the relevant capability slot API's `SlotEditLengthPolicy::FixedBudget` Fatal is that older matrix surface only; the canonical patch path is length-changing.
  - the relevant capability — RealLive runtime / VM port.
...
- [x] Crate-level provenance comment in `crates/kaifuu-reallive/src/lib.rs` extended with the the relevant capability paragraph. Per-module preambles updated in `encoding.rs`, `protected_spans.rs`, `bridge.rs`, `gameexe.rs`, `patchback/mod.rs`.
...
- [ ] If a future worker reads rlvm to confirm a hypothesis at ALPHA-006 (Sweetie HD evidence work), the readiness record records that fact with the file path consulted and the hypothesis confirmed, **without** importing rlvm's expression. (No such read was performed during the the relevant capability implementation slice.)

## the relevant capability length-changing patch-back addendum (`reallive-patchback-length-changing`)
