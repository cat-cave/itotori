...
| `reallive-seen-txt`                | RealLive        | `SEEN.TXT`, `SEEN.GAN`, `Gameexe.ini`, `*.g00`, `*.ovk`/`*.koe`/`*.nwk` aggregate presence; `*.pdt` informational cross-check for AVG32 lineage | `packed` when matched; `unknown_variant` with `kaifuu.ambiguous_engine_variant` when Siglus markers co-present; `unknown_variant` with `kaifuu.unsupported_engine_variant` when `.PDT` co-present and no RealLive-specific Gameexe.ini keys |
