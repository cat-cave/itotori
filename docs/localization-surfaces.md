...
| `variable_placeholder` | Runtime substitution such as player name, item count, currency, or stat.  | `spanKind`, `variableName`, `formatHint`, `exampleValues`, `startByte`, `endByte`, `preserveMode`.                                                                                               | Itotori may reorder or map it in target text, but patch exports must carry explicit `sourceSpanId`/source byte range and target byte range mappings when raw tokens can reorder or duplicate. |
