...
| Control-plane surface budget | Yes, directly in `_tier0.yml` before the dispatcher | Exact-count ratchet over 56 distinct prefixed names in tracked text and six parsed Just recipes; dynamic names, untracked files, recipe semantics, and other command surfaces are invisible; its regression tests are not selected                                                  |
...
| Public-lane coverage         | Yes                                                 | Ten hand-maintained categories require marker text and command tokens; it neither discovers nor executes cited tests                                                                                                                                                                |
...
| Coverage parity              | Yes                                                 | Hand-maintained manifest-group-to-function citations and function existence; it does not run or inspect the assertions                                                                                                                                                              |
...
| Mutation differential        | Tests only in meta; execution in Tier 1             | Ten fixed mutations for two engine families; no other mutation or required real-input corroboration                                                                                                                                                                                 |

Direct measurements were: 302/302 conventionally named suite files discoverable
by the live collection scanner, 22 public manifests, 10/10 public-lane marker
categories, and 100% of the enumerated synthetic component catalog. Those are
bounded results, not test-case or behavior execution
