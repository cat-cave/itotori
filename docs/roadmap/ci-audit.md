...
| Control-plane surface budget | Yes, directly in `_tier0.yml` before the dispatcher | Compares current tracked prefixed-name and parsed Just-recipe counts with `origin/main` merge-base; growth fails, while shrinkage is reported without blocking. Dynamic names, untracked files, recipe semantics, and other command surfaces are invisible; its regression tests are not selected |
