  ...
  private async runtimeDashboardArtifacts(
    ...
    return result.rows.map((row) => {
      ...
      return {
        ...
        uri,
        hash,
        hashProvenance: runtimeArtifactHashProvenanceFromRow(record.hash_provenance),
...
function artifactLinkFromRef(
  ...
  const hashProvenance = runtimeArtifactHashProvenance(artifactRef);
 ...
 */
function runtimeArtifactHashProvenance(
  artifactRef: RuntimeArtifactRefV02,
 ...
 */
function runtimeArtifactHashProvenanceFromRow(
  value: unknown,
): RuntimeArtifactHashProvenance | null {
  if (typeof value !== "string") {
    return null;
  }
  return RUNTIME_ARTIFACT_HASH_PROVENANCES.includes(value as RuntimeArtifactHashProvenance)
    ? (value as RuntimeArtifactHashProvenance)
    : null;
}
