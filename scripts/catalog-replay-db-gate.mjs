...
if (verifyOnlyResultsPath) {
  ...
  if (completeness.problems.length > 0) {
    printBanner([
      ...
      "a truncated / partial shared result set is NOT replay coverage",
      `remediation:      ${remediationCommand}`,
    ]);
    process.exit(1);
  }
} else {
  // Full mode: run ONLY the catalog replay suites against the DB.
  ...
  // Parse the machine-readable vitest report and PROVE each catalog replay
  // suite actually executed tests.
