...
function oneOf(kind, value, values) {
  if (!values.includes(value)) fail(`unknown ${kind} "${value}"; expected one of: ${values.join(", ")}`);
}

function check(scope, forwarded) {
  const scopes = ["all", "meta", "ts", "rust", "fixtures", "schema", "roadmap", "affected", "alpha-readiness", "rgt-readiness"];
  oneOf("check scope", scope, scopes);
  if (scope === "fixtures") return run("pnpm", ["exec", "node", "fixtures/validate-public-manifests.mjs", ...forwarded]);
  if (scope === "schema") return run("pnpm", ["exec", "node", "scripts/validate-impl-map-schema.mjs", ...forwarded]);
  ...
  if (scope === "alpha-readiness") return run("node", ["scripts/alpha-readiness-checklist.mjs", ...forwarded]);
  if (scope === "rgt-readiness") return run("node", ["scripts/rgt-readiness-checklist.mjs", ...forwarded]);
  if (scope === "ts") return shell("pnpm exec vp check\npnpm exec vp run ts:typecheck");
  if (scope === "rust") return shell("cargo fmt --check\ncargo check --workspace\ncargo clippy --workspace --all-targets --all-features -- -D warnings\ncargo deny check");
  if (scope === "meta") return shell(`
...
`);
  return shell(`node scripts/developer-command.mjs check meta\nnode scripts/developer-command.mjs check ts\nnode scripts/developer-command.mjs check rust`);
}

function test(selector, forwarded) {
  const selectors = ["all", "browser", "real-bytes", "real-bytes-oracle", "real-bytes-oracle-drift", "db", "permission-denial-db", "catalog-replay-db", "contract", "mutation-property", "mutation-differential", "alpha", "hello-replay", "hello-replay-validate", "ratio", "dlsite-demand"];
  oneOf("test selector", selector, selectors);
  if (selector === "all") return shell("pnpm exec vp run ts:test\ncargo test --workspace");
  if (selector === "browser") return shell(`
...
`);
  if (selector === "real-bytes") return shell("node scripts/real-bytes-lane.mjs\npnpm exec vp run ts:build\npnpm --filter @itotori/app exec vitest run test/rpgmaker-production-real-bytes.test.ts --exclude '**/.direnv/**'\npnpm --filter @itotori/app exec vitest run test/patchback-produce-build.test.ts --exclude '**/.direnv/**'\ncargo test -p kaifuu-core --test xp3_real_bytes_roundtrip\ncargo test -p kaifuu-reallive -p utsushi-reallive -p utsushi-siglus -p kaifuu-siglus -p kaifuu-cli -p utsushi-cli -p kaifuu-rpgmaker -p kaifuu-engine-fixture -- --ignored\ncargo test -p utsushi-core --test composite_asset_package_real_bytes\ncargo test -p kaifuu-vault-source --test live_vault_open_test --test live_vault_by_id_test --test live_vault_siglus_test -- --ignored\ncargo test -p kaifuu-softpal -- --ignored");
  if (selector === "real-bytes-oracle") return run("node", ["scripts/real-bytes-oracle.mjs", ...forwarded]);
  if (selector === "real-bytes-oracle-drift") return run("node", ["scripts/real-bytes-oracle.mjs", "--drift-only", ...forwarded]);
  if (selector === "db") return shell("rm -f .tmp/itotori-db/no-database-skipped.json\npnpm --filter @itotori/db test:db\nnode scripts/assert-db-tests-not-skipped.mjs");
  if (selector === "permission-denial-db") return run("node", ["scripts/permission-denial-db-gate.mjs", ...forwarded]);
  if (selector === "catalog-replay-db") return run("node", ["scripts/catalog-replay-db-gate.mjs", ...forwarded]);
  if (selector === "contract") return shell("pnpm --filter @itotori/localization-bridge-schema test\ncargo test -p kaifuu-core shared_contract_fixture_suite");
  if (selector === "mutation-property") return shell("pnpm exec vitest run packages/localization-bridge-schema/test/schema.test.ts\ncargo test -p kaifuu-core property");
  if (selector === "mutation-differential") return run("node", ["scripts/mutation-differential.mjs", ...forwarded]);
  if (selector === "alpha") return run("corepack", ["pnpm", "--dir", "apps/itotori", "exec", "vitest", "run", "test/composition-reachability.test.ts", "--exclude", "**/.direnv/**", ...forwarded]);
  if (selector === "hello-replay") return run("cargo", ["test", "-p", "utsushi-reallive", "--test", "replay_scene_synthetic", "--", "--nocapture", ...forwarded]);
  if (selector === "hello-replay-validate") return shell("cargo test -p utsushi-cli --bins replay_validate -- --nocapture\ncargo run -p utsushi-cli -- replay-validate --help");
  if (selector === "ratio") return run("node", ["scripts/classify-test-seams.mjs", ...forwarded]);
  return run("pnpm", ["exec", "vitest", "run", "apps/itotori/test/dlsite-demand.test.ts", "--exclude", "**/.direnv/**", ...forwarded]);
}

function ci(lane, forwarded) {
  const lanes = ["public", "affected", "tier0-meta", "tier0-ts", "tier0-rust", "tier0-manifest", "tier1-ts-public-1of2", "tier1-ts-public-2of2", "tier1-rust-1of3", "tier1-rust-2of3", "tier1-rust-3of3", "tier1-db", "tier1-browser", "tier1-mutation", "private-real-bytes"];
  oneOf("CI lane", lane, lanes);
  if (lane === "public") return shell("node scripts/developer-command.mjs check all\npnpm exec vp run ts:build\nnode scripts/developer-command.mjs test all\nnode scripts/developer-command.mjs test mutation-differential");
...
switch (delegate) {
  case "worktree-setup":
    ...
    break;
  case "dev":
    oneOf("development service", selector, ["app", "dashboard", "install", "build", "package", "package-pack", "scale-smoke", "scale-large", "roadmap-dashboard", "roadmap-dashboard-watch", "audit-findings-seed", "qd-import", "qd-export", "upgrade", "db-up", "db-down", "db-wait", "db-migrate", "db-reset"]);
    ...
    else if (selector === "package-pack") shell("node packages/itotori-cli/build.mjs\ncd packages/itotori-cli && npm pack");
    else if (selector === "scale-smoke" || selector === "scale-large") run("node", ["scripts/itotori-scale-harness.mjs", "--profile", selector.slice(6), ...args]);
    else if (selector === "roadmap-dashboard" || selector === "roadmap-dashboard-watch") shell(`pnpm --filter @itotori/spec-dag-dashboard build\nnode packages/spec-dag-dashboard/dist/cli.js ${selector.endsWith("watch") ? "--watch" : ""}`);
    else if (selector === "audit-findings-seed") shell("pnpm --filter @itotori/app build\nnode apps/itotori/dist/audit-findings/seed-cli.js");
    else if (selector === "qd-import") shell("qd import --from roadmap/spec-dag.json\nnode scripts/spec-dag.mjs validate\nqd doctor --json");
    else if (selector === "qd-export") shell("qd export --out roadmap/spec-dag.json\nnode scripts/spec-dag.mjs validate");
    else if (selector === "db-up") shell("node scripts/itotori-db-compose-env.mjs\ndocker compose --env-file \"$ITOTORI_DB_COMPOSE_ENV_PATH\" up -d postgres");
    else if (selector === "db-down") shell("node scripts/itotori-db-compose-env.mjs\ndocker compose --env-file \"$ITOTORI_DB_COMPOSE_ENV_PATH\" down");
    else if (selector === "db-wait") shell("node scripts/itotori-db-compose-env.mjs\nnode scripts/itotori-db-wait.mjs");
    else if (selector === "db-migrate") shell("pnpm exec vp run ts:build\nDATABASE_URL=\"$(node scripts/itotori-db-compose-env.mjs --print-database-url)\" node apps/itotori/dist/cli.js db-migrate");
    else if (selector === "db-reset") shell("pnpm exec vp run ts:build\nDATABASE_URL=\"$(node scripts/itotori-db-compose-env.mjs --print-database-url)\" node apps/itotori/dist/cli.js db-reset");
    else shell("corepack enable\nnode scripts/update-node-version.mjs\ncorepack use pnpm@latest\nnode scripts/sync-pnpm-engine.mjs\npnpm update --latest --recursive\nrustup update stable\ncargo update\nnode scripts/verify-toolchain-policy.mjs");
    break;
  case "doctor":
    oneOf("doctor profile", selector, ["core", "render", "full", "provision"]);
    run("node", ["scripts/native-deps.mjs", selector === "provision" ? "provision" : "doctor", ...(selector === "provision" ? args : [selector, ...args])]);
    break;
  case "check": check(selector, args); break;
  case "test": test(selector, args); break;
  case "ci": ci(selector, args); break;
  default: fail(`unknown delegate "${delegate}"; run just --summary`);
}
