...
test("RealLive runbook names the accepted-output production patchback seam and its local setup", () => {
  ...
  assert.match(
    runbook,
    /redacted `run-summary\.json` cannot be\s+fed to `patch` or `patch produce`/u,
  );
  assert.match(runbook, /Buffer\.alloc\(32, 11\)\.toString\("base64"\)/u);
  assert.match(
    runbook,
    /The OpenRouter provider key lives only in the main checkout's gitignored\s+`\.env`; it is never copied/u,
  );
  ...
  assert.doesNotMatch(
    readme,
    /no copy-paste CLI-only route from this archive to a localized patch/u,
  );
});
